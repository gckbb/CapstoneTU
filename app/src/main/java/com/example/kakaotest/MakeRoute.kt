...
class MakeRoute {
    ...
    fun findMinPoint(startRouteData:SearchRouteData): Boolean{
        ...
        for(i in 0 until routeList.count()) {
            tempTime = apiAdapter.apiRequest(startRouteData.pointdata.tpoint.longitude,
                startRouteData.pointdata.tpoint.latitude,
                routeList[i].pointdata.tpoint.longitude,
                routeList[i].pointdata.tpoint.latitude)?.toInt()
