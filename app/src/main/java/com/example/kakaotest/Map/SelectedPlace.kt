...
class SelectedPlace : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val   selectedPlaceList: ArrayList<SelectedPlaceData>? = SharedPreferenceUtil.getDataFromSharedPreferences(this)
            ...
            // 어댑터 생성 및 설정
            val selectedPlaceNames = selectedPlaceList?.map { "${it.placeName}" }?.toMutableList() ?: mutableListOf()

            // 어댑터 생성
            val nameAdapter =
                simpleListItem2Adapter(this, selectedPlaceList!!.toMutableList())
            ...
            nextButton.setOnClickListener {
                ...
                Log.d("Item", selectedPlaceList.toString())
            }
            ...
            foundListView.setOnItemClickListener { parent, view, position, id ->
                ...
                for ((key, value) in clickedRestaurantMap) {
                    if (value == clickedRestaurantName) {
                        ...
                        selectedPlaceList?.add(SelectedPlaceData(clickedRestaurantName,TMapPoint(longitude,latitude),address))
